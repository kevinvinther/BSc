...
In the TLA$^{+}$ verification suite, the TLC model is included. The model checker verifies the correctness of the algorithm by going through each possible state, and making sure that it produces satisfiable behavior, thus verifying if an algorithm works even in the cases that are hard to find using conventional tools \cite{10.1007/3-540-48153-2_6}.
...
In case the decision to kill the leader is not chosen, some process is chosen. The process either checks its leader or handles its messages.
...
The other reason might be something that I have not yet found, but has been a product of the interpretation work. I might have done too much interpretation, and really changed something did I did not think would change anything, or I did not realize to be a change.
